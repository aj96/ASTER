...
verbnet = nltk.corpus.VerbnetCorpusReader('../Event_Creation/tools/v3.3',
...
class eventMaker: 
	...
	# given a parse of a sentence, extract the event(s)
		...
		for (a,b,c,d) in zip(subjects, verbs, objects, modifiers):
            ...
            # NOTE: This is copied from repo that improved upon this one.
            # This repo only uses the first four tokens of an event
			self.events_original_words.append(returningEvent_orig[:-1])
			self.events.append(returningEvent[:-1])
...
input_text_file = open("input_train.txt", "w+")
output_text_file = open("output_train.txt", "w+")
for i, parse in enumerate(tqdm(PARSES)):
	...
	if sentence[-1] == ".":
		sentence = sentence[:-1]
	for event in maker.events_original_words:
		event = ' '.join(event)
		input_text_file.write(event + "\n")
		output_text_file.write(sentence + "\n")
