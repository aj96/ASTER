...
def hyperparam_string(config):
    ...
    exp_name += 'src_%s__' % (config['model']['src_type'])
    exp_name += 'trg_%s__' % (config['model']['trg_type'])
...
def read_data_pipeline(src, config, trg=None):
    print('Reading source data ...')
    src_lines = []
    src_lines.append(src.split().split())
    ...
    if trg is not None:
        print('Reading target data ...')
        trg_lines = []
        trg_lines.append(trg.strip().split())
