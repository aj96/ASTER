...
logging.info('Source Type : %s ' % (config['model']['src_type']))
logging.info('Target Type : %s ' % (config['model']['trg_type']))
...
for i in range(config['training']['epochs']):
    ...
    if i % config['management']['checkpoint_freq'] == 0:
        ...
        if config['management']['evaluate']:
            ...
            logging.info(' : %d Minibatch : %d : BLEU : %.5f ' %
                         (i, j, bleu))
