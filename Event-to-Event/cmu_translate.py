...
tf.app.flags.DEFINE_integer("in_vocab_size", 65000, "Input vocabulary size.")
tf.app.flags.DEFINE_integer("out_vocab_size", 65000, "Output vocabulary size.")
...
_buckets = [(28, 30), (10, 15), (20, 25), (40, 50)]
	...
	with tf.Session() as sess:
		...
		while epochs < FLAGS.max_epochs:
			...
			if epochs%10 == 0 and pos<FLAGS.batch_size:
				...
				checkpoint_path = os.path.join("/mnt/sdb1/EventEval/original/translate/checkpoints","all2sent_epoch{}.ckpt".format(epochs))
...
def decode():
	with tf.Session() as sess:
		...
		vocab_path = os.path.join(FLAGS.data_dir, "cmu_all2sent_translate.vocab")
		...
		test_dir = os.path.join(FLAGS.data_dir, "All2SentTest_input")
		pos = 0
		for sentence in open(test_dir+".txt"):
			token_ids = cmu_data_utils.sentence_to_token_ids(tf.compat.as_bytes(sentence), input_vocab)
			bucket_id = 1#min([b for b in xrange(len(_buckets))	if _buckets[b][0] > len(token_ids)])
